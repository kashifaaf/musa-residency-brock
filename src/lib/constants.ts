...
let __APP_URLInstance: ReturnType<typeof getAPP_URL> | null = null
...
export const APP_URL = new Proxy({} as ReturnType<typeof getAPP_URL>, {
  get(_, prop) {
    const target = getAPP_URL() as Record<string | symbol, unknown>
...
// Payment
let __STRIPE_PUBLISHABLE_KEYInstance: ReturnType<typeof getSTRIPE_PUBLISHABLE_KEY> | null = null
...
export const STRIPE_PUBLISHABLE_KEY = new Proxy({} as ReturnType<typeof getSTRIPE_PUBLISHABLE_KEY>, {
  get(_, prop) {
    const target = getSTRIPE_PUBLISHABLE_KEY() as Record<string | symbol, unknown>
    const value = target[prop]
    return typeof value === "function" ? value.bind(target) : value
  },
})
