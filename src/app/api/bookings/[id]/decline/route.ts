...
import { sendEmail, generateBookingDeclinedEmail } from '@/lib/email';
  ...
  try {
    ...
    const bookingData = await db
      .select({
        ...
        paymentIntentId: bookingRequests.paymentIntentId,
    ...
    await sendEmail({
      ...
      subject: 'Booking Request Update - Musa Residency',
      html: generateBookingDeclinedEmail(
        booking.guest.name,
        booking.home.title
      ),
    });
    ...
    return NextResponse.json({
      success: true,
      data: { message: 'Booking request declined successfully' },
    });
  } catch (error) {
