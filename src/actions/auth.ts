...
export async function createUser(formData: FormData): Promise<ActionResult<{ userId: string }>> {
  try {
    ...
    await sendEmail({
      ...
      text: `Please verify your email by clicking: ${process.env.NEXT_PUBLIC_APP_URL}/verify-email?token=${verificationToken}`,
      html: `
        <h1>Welcome to ${APP_NAME}!</h1>
        <p>Please verify your email by clicking the link below:</p>
        <a href="${process.env.NEXT_PUBLIC_APP_URL}/verify-email?token=${verificationToken}">Verify Email</a>
      `,
    });
